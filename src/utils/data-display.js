...
const { DATA_SOURCES, FIELD_TYPES, getSourceData: getData, getSourceDataKeys: getKeys } = require('../config/main');
...
function displaySourceData(result, selectedSource = 'all') {
  ...
  console.log('\n' + '='.repeat(80));
  console.log(`📊 WEEK SUMMARY RESULTS - Week ${result.weekNumber} of ${result.year}`);
  ...
  Object.entries(DATA_SOURCES).forEach(([sourceId, sourceConfig]) => {
    ...
    Object.entries(data).forEach(([dataKey, dataConfig]) => {
      ...
      console.log(`  ${dataConfig.label}: ${formattedValue}${suffix}`);
    });
  });
...
function collectSourceData(result, selectedSource = 'all') {
  // Handle array of results (multiple weeks)
  if (Array.isArray(result)) {
    return result.map((weekResult) => collectSourceData(weekResult, selectedSource));
  }
